      ...
      <div className={`group p-4 rounded-xl border backdrop-blur-md ${getSlotColor(block.category, block.priority)} transition-all relative flex flex-col`}>
        <div className="flex justify-between items-start w-full">
          <div className="flex items-center gap-2">
            ...
            <h4 className="font-semibold text-base md:text-lg leading-tight">{block.title}</h4>
          </div>
          
          <button 
            onClick={() => onEditClick(block)}
            className="md:opacity-0 opacity-100 group-hover:opacity-100 transition-opacity bg-white/10 hover:bg-white/20 p-1.5 rounded-lg text-white backdrop-blur-md shrink-0 ml-2"
          >
            <Edit3 size={16} />
          </button>
        </div>
...
const Timeline = () => {
  ...
  const timeline = hasPlanned ? generateDailyPlan(tasks, 9) : [];
    ...
    <div className="flex-1 flex flex-col min-h-0 relative">
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center mb-6 shrink-0 gap-4">
        ...
        <button 
          ...
          className="bg-primary hover:bg-primary/80 transition-all text-white px-5 py-2.5 sm:py-2 rounded-xl font-medium shadow-lg hover:shadow-[0_0_15px_rgba(59,130,246,0.5)] flex items-center justify-center gap-2 disabled:opacity-70 disabled:hover:shadow-none w-full sm:w-auto"
        >
      ...
      <TaskEditModal 
        key={editingTask?.id || 'task-edit-modal'}
