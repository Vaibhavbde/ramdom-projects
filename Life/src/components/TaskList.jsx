...
const SortableTask = ({ task, toggleTaskStatus, startFocus }) => {
  ...
  return (
    <div
      ...
      style={style}
      className={clsx(
        "group relative bg-black/40 border p-3 md:p-4 rounded-xl flex flex-col md:flex-row items-start gap-3 transition-colors",
    ...
    >
      <div className="flex items-start gap-2 md:gap-3 w-full">
        ...
        <div className="flex-1 min-w-0 md:pr-20">
          <h4 className="font-medium text-gray-100 truncate pr-2 md:pr-4 text-sm md:text-base">{task.title}</h4>
          <div className="flex flex-wrap items-center gap-x-2 md:gap-x-3 gap-y-2 mt-2 text-[10px] md:text-xs text-gray-500">
        ...
        <button
          ...
          className="hidden md:block opacity-0 group-hover:opacity-100 absolute right-4 top-[50%] -translate-y-[50%] bg-accent/20 text-accent hover:bg-accent hover:text-white transition-all px-3 py-1.5 rounded-lg text-xs font-medium shadow-[0_0_10px_rgba(16,185,129,0.2)]"
        >
          Focus
        </button>
      </div>
      ...
      <button
        onClick={() => startFocus(task)}
        className="md:hidden w-full mt-1 bg-accent/10 border border-accent/20 text-accent hover:bg-accent hover:text-white transition-all px-3 py-2 rounded-lg text-xs font-semibold flex justify-center items-center"
      >
        Start Focus
      </button>
    </div>
