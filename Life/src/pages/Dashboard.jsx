...
import { Compass, BarChart2, Plus, Zap, Target, User, Save, RefreshCw, Bell, Shield, TrendingUp, Clock, Activity, Download, CheckCircle2, LogOut, Menu } from 'lucide-react';

const Dashboard = () => {
  const { tasks, addTask, startFocus, activeTask } = useTasks();
  ...
  const renderContent = () => {
    switch(activeTab) {
      ...
      default: // Dashboard
        return (
          <div className="grid grid-cols-1 lg:grid-cols-3 gap-6 flex-1 min-h-0 overflow-y-auto lg:overflow-hidden pb-10 lg:pb-0 pr-1 lg:pr-0 custom-scrollbar">
            {/* Left Column: Tasks & Input */}
            <div className="lg:col-span-1 flex flex-col min-h-[500px] lg:min-h-0 relative z-10 w-full">
              <TaskInput />
              <TaskList />
            </div>

            {/* Right Column: Timeline Planner */}
            <div className="lg:col-span-2 glass-panel p-4 lg:p-6 rounded-2xl flex flex-col flex-1 min-h-[600px] lg:min-h-0 relative overflow-hidden bg-surface/40 w-full">
              <Timeline />
            </div>
          </div>
        );
    ...
    <div className="flex w-full h-full bg-background relative selection:bg-primary/30 font-sans">
      ...
      <div className="flex-1 flex flex-col h-full overflow-hidden p-4 md:p-6 md:pr-8 gap-6 relative">
        <header className="flex flex-col md:flex-row justify-between items-start md:items-center w-full shrink-0 gap-4 mb-2">
          ...
          <button 
            className="md:hidden p-2.5 bg-black/40 border border-white/10 rounded-xl text-white hover:bg-white/10 transition-colors shadow-lg"
          ...
          {/* Quick Actions Bar */}
          {activeTab === "Dashboard" && (
            <div className="flex bg-black/40 border border-white/10 rounded-xl overflow-hidden glass-panel p-1 w-full md:w-auto overflow-x-auto custom-scrollbar">
                <button 
                  onClick={() => document.getElementById('task-input')?.focus()}
                  className="px-3 py-1.5 flex items-center gap-2 text-xs font-semibold hover:bg-white/10 rounded-lg text-gray-300 transition-colors whitespace-nowrap"
                >
      ...
      {/* Absolute Overlays */}
      <FocusTimer key={activeTask?.id || 'focus-timer'} />
    </div>
  );
};
